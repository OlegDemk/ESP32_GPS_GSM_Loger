...
void task_gps_data(void *ignore)
{
	...
	gps_data_gps gps_data_gps_t;
	
	gps_data_gps_t.time.second = 0;
	gps_data_gps_t.time.minute = 0;
	gps_data_gps_t.time.hour = 0;
	
	gps_data_gps_t.date.day = 0;
	gps_data_gps_t.date.month = 0;
	gps_data_gps_t.date.year = 0;
	
	gps_data_gps_t.latitude = 24.01955;
	gps_data_gps_t.longitude = 49.80225;
	gps_data_gps_t.altitude = 250;
	gps_data_gps_t.speed = 0;
	gps_data_gps_t.sats_in_view = 7;
...
void task_BME280_data(void *ignore)
{
	...
	bme280_thp_queue_t.preassure = 0;
	
	while(1)
	{
		BaseType_t result = xQueueSend(bme280_queue, (void*)&bme280_thp_queue_t, pdMS_TO_TICKS(100));
		if(result == pdPASS)
		{
			ESP_LOGI(TAG_BME280, "Send data: T:%.1f, H:%.1f, P:%.1f", bme280_thp_queue_t.temperature, bme280_thp_queue_t.humidity, bme280_thp_queue_t.preassure);
		}
		else if(result == errQUEUE_FULL)
		{
			ESP_LOGE(TAG_BME280, "The queue is full");		// Черна заповнюється повністю бо не спрацьовує хендлер get_bme280_data_handler
		}
		else
		{
		...
		bme280_thp_queue_t.preassure = bme280_thp_queue_t.preassure + 1;
...
void task_reseive_data_data(void *ignore)
{
	...
	while(1)
	{
		...
		{
			ESP_LOGI(TAG_BME280, "Received data:  T:%.1f, H:%.1f, P:%.1f", bme280_thp_queue_t.temperature, bme280_thp_queue_t.humidity, bme280_thp_queue_t.preasure );
		}
